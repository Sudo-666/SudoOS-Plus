...
pub(crate) fn write_console_byte(byte: u8) {
    while read_line_status()
        & LINE_STATUS_TRANSMIT_EMPTY
        == 0
    {
        spin_loop();
    }
    ...
    let address = PhysAddr::new(
        UART_PHYS_BASE + UART_TRANSMIT_HOLDING,
    );

    let register =
        crate::memory::phys_access::mmio_mut_ptr::<u8>(
            address,
        )
...
fn read_line_status() -> u8 {
    let address = PhysAddr::new(
        UART_PHYS_BASE + UART_LINE_STATUS,
    );

    let register =
        crate::memory::phys_access::mmio_ptr::<u8>(
            address,
        )
        .expect("LoongArch UART is outside uncached DMW");
