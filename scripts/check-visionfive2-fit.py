...
用法:
    python3 scripts/check-visionfive2-fit.py \
        ...
        --dtb-contest-single build/visionfive2/tftp/sudoos/vf2/dtbs/vf2-contest-fixture-single.dtb \
        --dtb-contest-smp build/visionfive2/tftp/sudoos/vf2/dtbs/vf2-contest-fixture-smp.dtb \
        --dtb-finalall-single build/visionfive2/tftp/sudoos/vf2/dtbs/vf2-finalall-single.dtb \
        --dtb-finalall-smp build/visionfive2/tftp/sudoos/vf2/dtbs/vf2-finalall-smp.dtb \
...
def check_image(itb: Path, inputs: dict[str, Path]) -> None:
    ...
    # 全部 7 个 FDT 节点必须固定到 0x46000000:8 字节对齐(内核 valid_fdt_address
    # 要求)且与 tftp staging 解耦,避免 bootm 在 0x60xxxxxx 原地放未对齐 FDT。
    FDT_NAMES = ("fdt-selftest", "fdt-single", "fdt-smp",
                 "fdt-contest-single", "fdt-contest-smp",
                 "fdt-finalall-single", "fdt-finalall-smp")
    for name in FDT_NAMES:
        ...
        print(f"{name:<18}: load=0x{load:x}")
    ...
    if not hash_lines:
        ...
        if hash_count < len(blocks):
            raise CheckFailure(f"expected {len(blocks)} SHA-256 hash nodes, found {hash_count}")
    print(f"hash nodes      : SHA-256 present on all {len(blocks)} image blocks")
    ...
    # 逐字节比较提取结果:按节点名定位块,不能按固定下标——FIT 现含 7 个 FDT
    # 节点,ramdisk 位于 kernel + 7xFDT 之后(下标 8),早期 5 块的按序假定已失效。
    order = ([("kernel", "kernel-raw")]
             + [(name, f"dtb-{name[4:]}") for name in FDT_NAMES]
             + [("ramdisk", "initramfs")])
    for block_name, input_key in order:
        index = next(i for i, b in enumerate(blocks) if b.get("name") == block_name)
        ...
        print(f"extract[{index:>2}]   : {block_name} == {expected_path.name} "
              f"({len(extracted)} bytes, sha256 {sha256(expected_path)[:12]}…)")

    # 地址/大小不重叠:校验每个块文件大小总和可放入 staging 0x60000000。
    total = sum(len(extract(itb, i)) for i in range(len(blocks)))
    if total > 0x60000000:
...
def main() -> int:
    ...
    ap.add_argument("--dtb-contest-single", required=True)
    ap.add_argument("--dtb-contest-smp", required=True)
    ap.add_argument("--dtb-finalall-single", required=True)
    ap.add_argument("--dtb-finalall-smp", required=True)
    ...
    inputs = {
        ...
        "dtb-contest-single": Path(args.dtb_contest_single),
        "dtb-contest-smp": Path(args.dtb_contest_smp),
        "dtb-finalall-single": Path(args.dtb_finalall_single),
        "dtb-finalall-smp": Path(args.dtb_finalall_smp),
